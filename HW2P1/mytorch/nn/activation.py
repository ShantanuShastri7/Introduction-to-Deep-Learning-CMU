...
class Sigmoid:
    ...
    def backward(self, dLdA):
        ...
        return np.multiply(dLdA, (self.A - np.multiply(self.A, self.A)))
...
class Tanh:
    ...
    def backward(self, dLdA):
        return dLdA*(1 - np.power(self.A, 2))
...
class GELU:
    ...
    def forward(self, Z):
        self.Z = Z
        self.A = np.multiply(1/2*Z, (1 + scipy.special.erf(Z/np.sqrt(2))))
        return self.A
    
    def backward(self, dLdA):
        return np.multiply(dLdA, ((1/2*(1+scipy.special.erf(self.Z/np.sqrt(2)))) + np.multiply(self.Z/np.sqrt(2*np.pi), np.exp(-1*np.power(self.Z, 2)/2))))
...
class Swish:
    ...
    def forward(self, Z):
        self.Z = Z
        self.beta = 1
        self.A = np.multiply(Z, (1/(1+np.exp(-self.beta*Z))))
        return self.A
    
    def backward(self, dLdA):
        self.dLdbeta = np.sum(dLdA*self.Z*self.Z*((1/(1+np.exp(-self.beta*self.Z))))*(1-(1/(1+np.exp(-self.beta*self.Z)))))
        return dLdA*((1/(1+np.exp(-1*self.beta*self.Z))) + np.multiply(self.beta*self.Z , np.multiply(1/(1+np.exp(-self.beta*self.Z)), (1-(1/(1+np.exp(-self.beta*self.Z)))))))
