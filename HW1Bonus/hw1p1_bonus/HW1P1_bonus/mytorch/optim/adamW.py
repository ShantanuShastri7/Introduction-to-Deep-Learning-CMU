...
class AdamW():
    ...
    def step(self):
        ...
        for layer_id, layer in enumerate(self.l):
            # --- 1. Calculate updates for weights ---
            ...
            # --- 2. Calculate updates for biases ---
            ...
            # --- 3. Perform weight and bias updates with weight decay ---
            
            # In AdamW, weight decay is decoupled and applied directly to the weights.
            layer.W -= self.lr * (m_W_hat / (np.sqrt(v_W_hat) + self.eps) + self.weight_decay * layer.W)
            
            layer.b -= self.lr * (m_b_hat / (np.sqrt(v_b_hat) + self.eps) + self.weight_decay * layer.b)
