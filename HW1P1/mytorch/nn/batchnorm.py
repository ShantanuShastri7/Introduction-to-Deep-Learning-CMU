...
class BatchNorm1d:
    ...
    def forward(self, Z, eval=False):
        ...
        self.Z = Z
        self.N = Z.shape[0]  
        self.M = np.sum(Z, axis=0)/self.N  # TODO: Calculate mini-batch mean
        self.V = np.var(Z, axis=0)  # TODO: Calculate mini-batch variance

        if eval == False:
            # training mode
            self.NZ = (Z-self.M)/(np.sqrt(self.V + self.eps))  # TODO: Calculate the normalized input Ẑ
            self.BZ = self.NZ*self.BW + self.Bb  # TODO: Calculate the scaled and shifted for the normalized input Ẑ
            ...
            # inference mode
            self.NZ = (Z-self.running_M)/(np.sqrt(np.power(self.running_V, 2) + self.eps))  # TODO: Calculate the normalized input Ẑ using the running average for mean and variance
            self.BZ = self.NZ*self.BW + self.Bb  # TODO: Calculate the scaled and shifted for the normalized input Ẑ
    ...
    def backward(self, dLdBZ):
        ...
        dLdV = -0.5 * np.sum(dLdNZ * (self.Z - self.M) * np.power(self.V + self.eps, -1.5), axis=0)  # TODO: Compute gradient of loss backprop through variance calculation.
        dNZdM = -1.0 / np.sqrt(self.V + self.eps)  # TODO: Compute derivative of normalized input with respect to mean.
        ...
        dLdZ = (dLdNZ / np.sqrt(self.V + self.eps)) + \
               ...
               (dLdM / self.N)  # TODO: Compute gradient of loss with respect to the input.
